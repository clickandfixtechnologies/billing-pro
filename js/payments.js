...
CF.payments=(()=>{
 ...
 const applyAmount=async(invoice,customer,amount)=>{const allocated=Math.min(Math.max(0,Number(amount||0)),Math.max(0,Number(invoice.due||0)));invoice.due=CF.money(Number(invoice.due||0)-allocated);invoice.paid=CF.money(Number(invoice.paid||0)+allocated);invoice.paymentStatus=invoice.due===0?"Paid":invoice.paid>0?"Partial":"Unpaid";customer.outstanding=CF.money(Number(customer.outstanding||0)-Number(amount||0));await Promise.all([CF.db.put("invoices",invoice),CF.db.put("customers",customer)]);return allocated};

 const saveEdit=async(e,p)=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target)),amount=Number(d.amount);if(!(amount>0))return CF.toast("Enter a valid payment amount","error");const [oldInvoice,oldCustomer,targetInvoice]=await Promise.all([CF.db.get("invoices",p.invoiceId),CF.db.get("customers",p.customerId),CF.db.get("invoices",d.invoiceId)]);if(!targetInvoice)return CF.toast("Select a valid invoice","error");try{const previouslyApplied=Number((p.appliedAmount??p.amount)||0);if(oldInvoice&&oldCustomer){oldInvoice.due=CF.money(Number(oldInvoice.due||0)+previouslyApplied);oldInvoice.paid=CF.money(Math.max(0,Number(oldInvoice.paid||0)-previouslyApplied));oldInvoice.paymentStatus=oldInvoice.due===0?"Paid":oldInvoice.paid>0?"Partial":"Unpaid";oldCustomer.outstanding=CF.money(Number(oldCustomer.outstanding||0)+Number(p.amount||0));await Promise.all([CF.db.put("invoices",oldInvoice),CF.db.put("customers",oldCustomer)]);}const invoice=await CF.db.get("invoices",d.invoiceId),customer=await CF.db.get("customers",invoice.customerId),appliedAmount=await applyAmount(invoice,customer,amount);Object.assign(p,d,{amount,appliedAmount,advanceAmount:CF.money(amount-appliedAmount),customerId:invoice.customerId,updatedAt:new Date().toISOString()});await CF.db.put("payments",p);CF.toast("Payment updated and balances adjusted");CF.router.render()}catch(error){CF.toast(error.message||"Could not update payment","error")}};
 ...
 const removePayment=async id=>{
  ...
  try {
   ...
   if(invoice && customer) {
     const appliedAmount = Number((p.appliedAmount ?? p.amount) || 0);
     invoice.due = CF.money(Number(invoice.due || 0) + appliedAmount);
     invoice.paid = CF.money(Math.max(0, Number(invoice.paid || 0) - appliedAmount));
 ...
 const save=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target)),invoice=await CF.db.get("invoices",d.invoiceId),amount=Number(d.amount);if(!invoice||!(amount>0))return CF.toast("Select an invoice and enter a valid amount","error");const c=await CF.customerAccounts.getCurrent(invoice.customerId);try{const appliedAmount=await applyAmount(invoice,c,amount),p={paymentId:CF.id(),...d,amount,appliedAmount,advanceAmount:CF.money(amount-appliedAmount),customerId:invoice.customerId,createdAt:new Date().toISOString()};await CF.db.put("payments",p);CF.toast(p.advanceAmount>0?`Payment recorded; ${CF.formatCurrency(p.advanceAmount)} kept as customer advance.`:"Payment recorded");await slip(p.paymentId);CF.router.render()}catch(error){CF.toast(error.message||"Could not record payment","error")}};
